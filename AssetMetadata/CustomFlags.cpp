...
namespace Pipeline {
	namespace CustomFlags {
		...
		Enum getEnum(const char* name) { return Enum(static_cast<uint64>(indexToValue((indexOf(name, names, count))))); }
		bool setName(const char* name, uint64* const value)
		{
			const char index = indexOf(name, names, count);
			if (index)
			{
				*value |= static_cast<uint64>(indexToValue(index));
				return true;
			}
			return false;
		}
		bool setNames(std::vector<std::string>& new_names, uint64* const value)
		{
			...
			for (; index != indices.end(); ++index) new_values |= static_cast<uint64>(indexToValue(*index));
			*value |= new_values;
			return true;
		}
